...
function CTAs({ dark = false }: { dark?: boolean }) {
  if (dark) {
    return (
      <>
        <a href="#schedule" className="hidden md:inline-block rounded-md border border-white bg-transparent px-3 py-2 text-sm font-semibold text-white hover:bg-white/5 transition-colors duration-360">
          {CTA_SCHEDULE}
        </a>
  ...
  return (
    <>
      <a href="#schedule" className="hidden md:inline-block rounded-md border border-primary bg-white px-3 py-2 text-sm font-semibold text-primary hover:bg-primary/5 transition-colors duration-360">
        {CTA_SCHEDULE}
      </a>
    </>
  );
  ...
  return (
    <>
      ...
      <header className="site-header w-full relative">
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 text-zinc-900 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Logo className="sm:hidden" size="h-15" />
            {/* <Logo className="hidden sm:flex" size="h-12" /> */}
          </div>

          <NavLinks menu={menu} className="hidden lg:flex items-center gap-4 text-sm" dark={false} />
      ...
      {scrolled && (
        <header className="compact-header fixed top-0 left-0 right-0 z-50 w-full backdrop-blur-sm" style={{ backgroundColor: 'var(--bg-dark)' }}>
          <div className="mx-auto max-w-7xl px-4 flex items-center justify-between text-white">
            <Logo size="h-15" />
          ...
          {open && (
            <div id="mobile-menu-compact" className="lg:hidden border-t" style={{ backgroundColor: 'var(--bg-dark)' }}>
              <div className="mx-auto max-w-7xl px-4 py-4 space-y-3 text-white">
                <div className="flex flex-col gap-2">
                  {menu.map((m) => (
                    <Link key={m.label} href={m.href} onClick={() => setOpen(false)} className="block rounded-md px-3 py-2 text-base font-medium text-white hover:bg-white/5 hover:text-primary uppercase tracking-wide transition-colors duration-150">
